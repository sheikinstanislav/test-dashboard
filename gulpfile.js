...
const paths = {
  html: {
    src: [`${srcFolder}/*.html`, `${srcFolder}/**/*.php`],
    watch_srs: [`${srcFolder}/**/*.html`, `${srcFolder}/**/*.ejs`],
    dest: `${buildFolder}/`,
  },
  scss: {
    src: [
...
function cleanDist() {
  return gulp.src(paths.img.srcForConversion).pipe(
    plumber({
      ...
      .pipe(
        gulpif(
          ...
          htmlhint({
            ...
            'tagname-lowercas': false,
            'tagname-specialchars': false, // ID
            'id-unique': false, // Attributes
            'alt-require': false,
          })
        )
      )
...
function files(callback) {
  ...
  // Vendors
  gulp.src(paths.vendors.src).pipe(gulp.dest(paths.vendors.dest));

  // Videos
  gulp.src(paths.video.src).pipe(gulp.dest(paths.video.dest));
...
function imgOptimization() {
  return (
    gulp
      .src(paths.img.srcForOptimization)
      .pipe(
        plumber({
          errorHandler: function (err) {
            notify.onError({
...
function imgWebPConversion() {
  // WebP
  return gulp
    .src(paths.img.srcForConversion)
    .pipe(
      plumber({
        errorHandler: function (err) {
          notify.onError({
...
function watch() {
  ...
  // Images
  gulp.watch(
    paths.img.srcForOptimization,
    gulp.series(imgOptimization, imgWebPConversion, imgAvifConversion)
  );
...
const dev = gulp.series(
  setDevelopmentEnvironment,
  cleanDist,
  gulp.parallel(files, scss, js, img),
  html,
